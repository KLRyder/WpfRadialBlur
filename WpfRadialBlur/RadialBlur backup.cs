...
{
    public class RadialBlurOld
    {
        public static RadialBitmap ConvertToRadial(Bitmap img)
        {
            var destImg = new Bitmap((img.Height + img.Width - 2) * 2,
                (int) Math.Ceiling(Math.Sqrt(img.Height * img.Height + img.Width * img.Width) / 2));

            var centerPoint = new Vector(img.Width / 2.0, img.Height / 2.0);
            for (var xIndex = 0; xIndex < destImg.Width; xIndex++)
            {
                var directionVector = DirectionVector(xIndex, img, centerPoint);

                for (var yIndex = 0; yIndex < destImg.Height; yIndex++)
                {
                    var point = centerPoint + directionVector * (yIndex + 1);
                    var c = InterperlatePointFromImg(img, point);
                    try
                    {
                        destImg.SetPixel(xIndex, yIndex, c);
                    }
                    catch (Exception e)
                    {
                        MessageBox.Show("X = " + xIndex + " Width = " + destImg.Width + "\nY = " + yIndex +
                                        " Hight = " +
                                        destImg.Height + "\n" + e.Message);
                    }
                }
            ...
            return new RadialBitmap(destImg, img.Width, img.Height);
        }
